...
# greets https://gist.github.com/peterhost/715255
...
DAEMON_ARGS="/home/ld36/plivo/call.js"      # path to your node.js server/app
                                        # NB: don't use ~/ in path

DESC="node.js http server"                  # whatever fancy description you like

NODEUSER=ld36:ld36                     # USER who OWNS the daemon process (no matter whoever runs the init script)
                                        # user:group (if no group is specified, the primary GID for that user is used)

LOCAL_VAR_RUN=/home/ld36/var/run            # in case the init script is run by non-root user, you need to
...
DAEMON=/home/ld36/.nvm/versions/node/v4.5.0/bin/$NAME                 # this SHOULD POINT TO where your node executable is
#
#                                                                              #
